...
class Building2Page extends StatefulWidget {
  ...
  static const List<DataColumn> _columns = [
    ...
    DataColumn2(label: Text("ID Gedung"), size: ColumnSize.S),
    DataColumn2(label: Text("Nama"), size: ColumnSize.L),
    DataColumn2(label: Text("Aksi")),
  ];
}

class _Building2PageState extends State<Building2Page> {
                ...
                () => Container(
                  ...
                  child: SizedBox(
                    ...
                    child: DataTable2(
                        ...
                        horizontalMargin: 12,
                        minWidth: 600,
                        columns: Building2Page._columns,
                        rows: _dataTable()),
                  ),
                ),
              ),
  ...
  List<DataRow> _dataTable() {
    return List<DataRow>.generate(
      _controller.buildingList.length,
      (index) => DataRow(
        cells: [
          DataCell(CustomText(text: (index + 1).toString())),
          DataCell(
            CustomText(text: _controller.buildingList[index].id.toString()),
          ),
          DataCell(
            CustomText(text: _controller.buildingList[index].name.toString()),
          ),
          DataCell(
            Row(
              children: [
