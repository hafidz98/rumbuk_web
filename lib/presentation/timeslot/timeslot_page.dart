...
class TimeSlotPage extends StatefulWidget {
  ...
  static const List<DataColumn> _columns = [
    ...
    DataColumn2(label: Text("Durasi (menit)"), size: ColumnSize.S),
...
class _TimeSlotPageState extends State<TimeSlotPage> {
  ...
  List<DataRow> _dataTable() {
    return List<DataRow>.generate(
      _controller.timeSlot.length,
      (index) => DataRow(
        cells: [
          DataCell(CustomText(text: (index + 1).toString())),
          DataCell(
            CustomText(text: _controller.timeSlot[index].id.toString()),
          ),
          DataCell(
            CustomText(text: _controller.timeSlot[index].startTime.toString()),
          ),
          DataCell(
            CustomText(text: _controller.timeSlot[index].endTime.toString()),
          ),
          DataCell(
            CustomText(text: _controller.timeSlot[index].duration.toString()),
          ),
          DataCell(
            Row(
              children: [
