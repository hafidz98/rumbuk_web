...
class RoomPage extends StatefulWidget {
  ...
  static const List<DataColumn> _columns = [
    ...
    DataColumn2(label: Text("Kapasitas")),
    DataColumn2(label: Text("Aksi")),
    DataColumn2(label: Text("Aktif")),
  ];
}
        ...
        (index) => DataRow(
              ...
              cells: [
                ...
                DataCell(Transform.scale(
                  ...
                  child: Obx(
                    () => Switch(
                      value: _controller.roomList[index].status == "1"
                          ? true
                          : false,
                      activeColor: Colors.green,
                      onChanged: (value) {
                        setState(() {
                          //_switchValue = value;
                          var id = _controller.roomList[index].id;
                          var status = value ? "1" : "0";
                          _controller.setActiveStatus(id, status);
                          _controller.refresh();
                          CustomSnackBar.of(context)
                              .customSnackBar('Tersimpan');
                        });
                      },
                    ),
                  ),
